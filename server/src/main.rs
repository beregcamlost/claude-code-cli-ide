...
use websocket::{cleanup_lock_file, run_websocket_server_full, run_websocket_server_with_worktree};
...
async fn main() -> Result<()> {
    ...
    match cli.mode {
        ...
        Some(Mode::Websocket { port, worktree }) => {
            let worktree_path = cli.worktree.or(worktree);
            run_websocket_server_with_worktree(port, worktree_path).await
        }
